...
***
This paper presents a methodology to "scale-up" crowdsourced commuter movement data from a small sample to the entire working population, building on techniques developed in the field of 'spatial microsimulation'. The motivation for the method is that upscaled data can be used as a basis for Agent-Based Modelling (ABM), e.g. to model travel patterns and simulate scenarios of change. The method builds on a spatial microdataset of residential populations in which agents are categorised by demographic, geographic, and socioeconomic variables, including: age, gender, in addition to home and work locations derived from origin-destination data. Further, data on mode of travel and economic activity are included to illustrate the method's potential for modelling targetted policy interventions. Case studies from Newcastle and Ilkley, UK, demonstrate the method's scalability nationally. To enable reproducibility and ease extensions to the method, the code underlying the MSMCity method has been packaged into an R package of the same name, to automate the process as far as is possible. We conclude that the method and represents a step towards the long sought-after integration of the fields of spatial microsimulation and agent-based modelling. Future work could build on the method to simulate settlement travel patterns anywhere in the world using global datasets such as OpenStreetMap.  

***

# Overview

This work focusses on the commute patterns of individuals on a large scale, with the intent of being able to provide insight to transport planners on areas in which infrastructure improvements can be made.
