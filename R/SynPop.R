...
# TODO add travel mode from census data (see getWorkTravelModes in api/NomiswebApi.R)

getSynPop = function(region) {
  ...
  # Get msoaPop - age by sex by econ ca=t
  regionPop = getEconData("NM_750_1", origins, sexes, ages, economic_activity, columns)

  # broaden age ranges
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 16 to 19"
                  | regionPop$C_AGE_NAME == "Age 20 to 21"
                  | regionPop$C_AGE_NAME == "Age 22 to 24" ] = "16-24"
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 25 to 29"
                  | regionPop$C_AGE_NAME == "Age 30 to 34" ] = "25-34"
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 35 to 39"
                  | regionPop$C_AGE_NAME == "Age 40 to 44" ] = "35-44"
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 45 to 49"
                  | regionPop$C_AGE_NAME == "Age 50 to 54" ] = "45-54"
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 55 to 59"
                  | regionPop$C_AGE_NAME == "Age 60 to 64" ] = "55-64"
  regionPop$C_AGE_NAME[regionPop$C_AGE_NAME == "Age 65 and over" ] = "65+"

  # shorten sex names
  regionPop$C_SEX_NAME[regionPop$C_SEX_NAME == "Females"] = "F"
  regionPop$C_SEX_NAME[regionPop$C_SEX_NAME == "Males"] = "M"

  # shorten econ act names
  regionPop$ECONOMIC_ACTIVITY_NAME[regionPop$ECONOMIC_ACTIVITY_NAME == "Economically active: In employment: Employee: Full-time (including full-time students)"] = "E FT"
  regionPop$ECONOMIC_ACTIVITY_NAME[regionPop$ECONOMIC_ACTIVITY_NAME == "Economically active: In employment: Employee: Part-time (including full-time students)"] = "E PT"
  regionPop$ECONOMIC_ACTIVITY_NAME[regionPop$ECONOMIC_ACTIVITY_NAME == "Economically active: In employment: Self-employed: Full-time (including full-time students)"] = "S FT"
  regionPop$ECONOMIC_ACTIVITY_NAME[regionPop$ECONOMIC_ACTIVITY_NAME == "Economically active: In employment: Self-employed: Part-time (including full-time students)"] = "S PT"

  # collapse age groups
  regionPop=summarise(group_by(regionPop, GEOGRAPHY_CODE , C_SEX_NAME, C_AGE_NAME, ECONOMIC_ACTIVITY_NAME), sum(OBS_VALUE))
  ...
  #msoa = "E02001718" {
    ...
    msoaPop$`sum(OBS_VALUE)`=NULL

    #ageSexEcon = msoaPop$`sum(OBS_VALUE)`
    # SURELY a better way to do this...
    sex = c(sum(msoaPop[msoaPop$C_SEX_NAME=="F",]$OBS_VALUE),sum(msoaPop[msoaPop$C_SEX_NAME=="M",]$OBS_VALUE))

    age = c(sum(msoaPop[msoaPop$C_AGE_NAME=="16-24",]$OBS_VALUE),
            sum(msoaPop[msoaPop$C_AGE_NAME=="25-34",]$OBS_VALUE),
            sum(msoaPop[msoaPop$C_AGE_NAME=="35-44",]$OBS_VALUE),
            sum(msoaPop[msoaPop$C_AGE_NAME=="45-54",]$OBS_VALUE),
            sum(msoaPop[msoaPop$C_AGE_NAME=="55-64",]$OBS_VALUE),
            sum(msoaPop[msoaPop$C_AGE_NAME=="65+",]$OBS_VALUE))

    econ = c(sum(msoaPop[msoaPop$ECONOMIC_ACTIVITY_NAME=="E FT",]$OBS_VALUE),
             sum(msoaPop[msoaPop$ECONOMIC_ACTIVITY_NAME=="E PT",]$OBS_VALUE),
             sum(msoaPop[msoaPop$ECONOMIC_ACTIVITY_NAME=="S FT",]$OBS_VALUE),
             sum(msoaPop[msoaPop$ECONOMIC_ACTIVITY_NAME=="S PT",]$OBS_VALUE))
    ...
    if (debug) {
      ...
      stopifnot(sum(synPop[synPop$Dest==od$PLACE_OF_WORK_CODE[1],]$NumPeople)==dests[1])
      stopifnot(sum(synPop[synPop$Econ=="E FT",]$NumPeople)==econ[1])
      stopifnot(sum(synPop[synPop$Sex=="F",]$NumPeople)==sex[1])
      stopifnot(sum(synPop[synPop$Age=="16-24",]$NumPeople)==age[1])
    }

    synPop=rbind(synPop, msoaSynPop)
  }
