...
origins <- getMSOAs(paste0(region, "*"))
# All
...
# define table
allSynPop<-data.frame(Origin=character(), Dest=character(), Age=character(), Sex=character(), Econ=character(), NumPeople=numeric(), stringsAsFactors = FALSE)
...
for (msoa in originMsoas) {
  ...
  sex = c(sum(pop[pop$C_SEX_NAME=="F",]$OBS_VALUE),sum(pop[pop$C_SEX_NAME=="M",]$OBS_VALUE))

  age = c(sum(pop[pop$C_AGE_NAME=="16-24",]$OBS_VALUE),
          sum(pop[pop$C_AGE_NAME=="25-34",]$OBS_VALUE),
          sum(pop[pop$C_AGE_NAME=="35-44",]$OBS_VALUE),
          sum(pop[pop$C_AGE_NAME=="45-54",]$OBS_VALUE),
          sum(pop[pop$C_AGE_NAME=="55-64",]$OBS_VALUE),
          sum(pop[pop$C_AGE_NAME=="65+",]$OBS_VALUE))

  econ = c(sum(pop[pop$ECONOMIC_ACTIVITY_NAME=="E FT",]$OBS_VALUE),
           sum(pop[pop$ECONOMIC_ACTIVITY_NAME=="E PT",]$OBS_VALUE),
           sum(pop[pop$ECONOMIC_ACTIVITY_NAME=="S FT",]$OBS_VALUE),
           sum(pop[pop$ECONOMIC_ACTIVITY_NAME=="S PT",]$OBS_VALUE))
